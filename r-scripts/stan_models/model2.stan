...
parameters {
  ...
  vector<lower = 0>[K] beta; // the regression coefficients
...
model {
  ...
  // Define the priors
  beta[1] ~ normal(200, 20); // same prior for all betas; we could define a different one for each, or use a multivariate prior
  beta[2] ~ normal(1, 0.5);
  sigma ~ cauchy(0, 2.5);
