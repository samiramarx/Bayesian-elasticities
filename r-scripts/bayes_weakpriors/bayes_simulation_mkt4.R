...
# BAYES ESTIMATION - MKT 4 - WEAKLY INFORMATIVE PRIOR
#--------------------------------------
# library
#--------------------------------------
library(tidyverse)
...
mkt4_v1f <- stan(file = "scripts/stan_models/stan_model_mkt4_v1f.stan", 
                 data = list(X = X, N = n, K = k, 
                             y = y_v1f), 
                 cores = cores, 
                 chains = chains, 
                 iter = iter)
save(mkt4_v1f, file = "bayes_simulation_mkt4_v1f.RData")

# V1R
y_v1r <- log(dta$jny_1r)
mkt4_v1r <- stan(file = "scripts/stan_models/stan_model_mkt4_v1r.stan", 
                 data = list(X = X, N = n, K = k, 
                             y = y_v1r), 
                 cores = cores, 
                 chains = chains, 
                 iter = iter)
save(mkt4_v1r, file = "bayes_simulation_mkt4_v1r.RData")

# V1A
y_v1a <- log(dta$jny_1a)
mkt4_v1a <- stan(file = "scripts/stan_models/stan_model_mkt4_v1a.stan", 
                 data = list(X = X, N = n, K = k, 
                             y = y_v1a), 
                 cores = cores, 
                 chains = chains, 
                 iter = iter)
save(mkt4_v1a, file = "bayes_simulation_mkt4_v1a.RData")

# V2F
y_v2f <- log(dta$jny_2f)
mkt4_v2f <- stan(file = "scripts/stan_models/stan_model_mkt4_v2f.stan", 
                 data = list(X = X, N = n, K = k, 
                             y = y_v2f), 
                 cores = cores, 
                 chains = chains, 
                 iter = iter)
save(mkt4_v2f, file = "bayes_simulation_mkt4_v2f.RData")

# V2R
y_v2r <- log(dta$jny_2r)
mkt4_v2r <- stan(file = "scripts/stan_models/stan_model_mkt4_v2r.stan", 
                 data = list(X = X, N = n, K = k, 
                             y = y_v2r), 
                 cores = cores, 
                 chains = chains, 
                 iter = iter)
save(mkt4_v2r, file = "bayes_simulation_mkt4_v2r.RData")

# V2A
y_v2a <- log(dta$jny_2a)
mkt4_v2a <- stan(file = "scripts/stan_models/stan_model_mkt4_v2a.stan", 
                 data = list(X = X, N = n, K = k, 
                             y = y_v2a), 
                 cores = cores, 
                 chains = chains, 
                 iter = iter)
save(mkt4_v2a, file = "bayes_simulation_mkt4_v2a.RData")
