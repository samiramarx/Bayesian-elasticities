...
mkt1_v2f <- stan(file = "scripts/stan_models/stan_model_mkt1_v2f.stan", 
                   data = list(X = X, N = n, K = k, 
                               y = y_v2f), 
                   cores = cores, 
                   chains = chains, 
                   iter = iter)
save(mkt1_v2f, file = "bayes_simulation_mkt1_v2f.RData")

# V2R
y_v2r <- log(dta$jny_2r)
mkt1_v2r <- stan(file = "scripts/stan_models/stan_model_mkt1_v2r.stan", 
                   data = list(X = X, N = n, K = k, 
                               y = y_v2r), 
                   cores = cores, 
                   chains = chains, 
                   iter = iter)
save(mkt1_v2r, file = "bayes_simulation_mkt1_v2r.RData")
#--------------------------------------
# save(mkt1_v2f, mkt1_v2r, file = "bayes_simulation_mkt1.RData")
